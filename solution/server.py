...
class Personaje():
    def __init__(self):
        ...
        self.dexterity=None
    def __str__(self):
        return f"Name:{self.name} Level: {self.level} Role: {self.role} Charisma: {self.charisma} Strenght: {self.strength} Dexterity: {self.dexterity}"
...
class VideojuegoService:
    ...
    def create_personaje(self, post_data):
        ...
        strenght = post_data.get("strenght",None)
        ...
        if len(personajes)==0:
            ...
            ca=0
            for index in personajes:
                ca=index
            personajes[ca+ 1] = personaje
        return personaje

    def read_personajes(self):
        return {index: personaje.__dict__ for index, personaje in personajes.items()}
    def filter_by_role_nivel_charisma(self,role,level,charisma):
        return {index: personaje.__dict__ for index, personaje in personajes.items() if personaje.role==role and personaje.level==int(level) and personaje.charisma==int(charisma)}
    def find_personaje(self,id):
        return {index: personaje.__dict__ for index, personaje in personajes.items() if index==id}
    ...
    def do_GET(self):
        ...
        if parsed_path.path == "/characters":
            if "role" in query_params and "level" in query_params and "charisma" in query_params:
                ...
                personajes_filtrados = self.controller.filter_by_role_nivel_charisma(role,level,charisma)
                HTTPDataHandler.handle_response(self,200,personajes_filtrados)
            ...
            personaje = self.controller.find_personaje(id)
            if personaje:
